...
var goEasyMessage = new GoEasy({
    appkey: 'BC-56a6cb94fd9b4e8ea34a8d0a9295d37e'
});
var goEasyApprove = new GoEasy({
    appkey: 'BC-7338c9d31b5c40188b3ac09955b5e96e'
});
...
class PersonPageComp extends Component {
    ...
    jumpLink=(item)=>{
        switch (item.urlCode) {
            ...
            case "14":
                return <a className="sendLink" target="_blank" href={`/wf/mobile/apply/info?wfsId=`+item.orderId} dangerouslySetInnerHTML={{ __html: item.senderMsg}}></a>
            default:
    ...
    render() {
        ...
        return (
            ...
            <div className="person">
                ...
                {this.state.hideNotice?<div className="system-notice"><i className="c2mfont c2m-ec-notice blue-gonggao" style={{paddingRight:7}}></i>系统公告：<a className="notice-title" href={`${prefixCh}announce.html#/${noticeId}`}>{noticeTitle}</a><span style={{paddingLeft:20}}>{noticeDate}</span><a style={{paddingLeft:20}} href={`${prefixCh}R/setting/announcement`}>更多>></a></div>:null}
                <div className="main">
